...
def main():     
    ...
    model = load_model(BASE_MODEL, FINETUNED_MODEL)
    model.eval()
        ...
        for video in os.listdir(f'{TENSOR_DIR}/{participant}'):
            ...
            with h5py.File(f'{FEATURE_DIR}/{participant}/{video}.h5', 'w') as file:
                for i, clip_batch in enumerate(f'{TENSOR_DIR}/{participant}/{video}'):
                    ...
                    current_batch = current_batch.unsqueeze(0)
                    features = model(current_batch)[0]
                    file.create_dataset(f'clip_{i}', data = features)
                    del current_batch
